...
interpolation=cv2.INTER_CUBIC)
    ...
    def process_frame(self, frame, side):
        ...
        for agent_row in range(0, 5):
            cropped_cred_image = frame[y_start:y_end, 1200:1321]
            cv2.imshow("Image", cropped_cred_image)
        #cv2.waitKey()
        #pytesseract.pytesseract.tesseract_cmd = "C:\\Program Files\\Tesseract-OCR\\tesseract.exe"
        #points = pytesseract.image_to_string(cropped_cred_image, lang='eng', config='--psm 10 --oem 3 -c tessedit_char_whitelist=0123456789')
        #print("points: ", points)
        cpoints = self.reader.readtext(self.clean_frame(cropped_cred_image),allowlist ='0123456789')
        cpoints = {"number": cpoints[0][1], "confidence": cpoints[0][2]}
        #print(cpoints)
...
if __name__ == "__main__":
    ...
    for i in range(2, 10):
        ...
        identified_credits=get_Credits.get_creds(image)
        print("identified Credits", identified_credits)
        end = time.time()
